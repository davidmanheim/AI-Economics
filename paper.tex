...
$v_m$ & Price-taking marginal net revenue per FLOP for model $m$ \\
...
$r^W$ & Competitive-benchmark (price-taking) clearing value of a FLOP \\
$r^A$ & Oligopoly/auction market-clearing wholesale price of a FLOP \\
...
$\rho$ & Discount rate (Secs.~\ref{sec:model-race}, \ref{sec:natural-oligopoly}); distinct from the wholesale compute prices $r^W,r^A$ \\
$\lambda_h$ & Hyperscaler $h$'s internal shadow value of a FLOP, net of $c$ (Sec.~\ref{sec:vertical-integration}) \\
$\chi_{\ell j}$ & Diversion harm to affiliated laboratory $\ell$ per FLOP supplied to rival $j$ (Sec.~\ref{sec:vertical-integration}) \\
$\omega$ & Hyperscaler equity share in an affiliated laboratory (Sec.~\ref{sec:vertical-integration}) \\
$K_R$ & Capacity reserved for an affiliated laboratory before the residual market clears (Sec.~\ref{sec:vertical-integration}) \\
$\tilde r$ & Internal transfer price for compute, an accounting object (Sec.~\ref{sec:vertical-integration}) \\
\hline
...
$\chi_{\ell,k}$ (task comparative advantage, Sec.~\ref{sec:persistent-advantage}) vs.\ $\chi_{\ell j}$ (diversion harm, Sec.~\ref{sec:vertical-integration}) & distinguished by index: $k$ ranges over tasks, $j$ over rival laboratories \\
$\gamma$ (output-unit rescaling, App.~\ref{app:extensions}) vs.\ $\Gamma_\ell$ (entry-cost inverse, Sec.~\ref{sec:natural-oligopoly}) & distinct; the rescaling use is a bare scalar \\
...
\begin{equation}\Pi(\mathbf y)=R(\mathbf y;\mathbf Q)-\sum_{m=1}^{M}b_m\cdot y_m-c\cdot\sum_{m=1}^{M}a_m\cdot y_m.\end{equation}
...
Let $r^W$ be the market-clearing wholesale value of a FLOP in the competitive-allocation benchmark. Every active laboratory satisfies

\begin{equation}v_\ell(x_\ell)=r^W,\end{equation}

while inactive laboratories satisfy

\begin{equation}v_\ell(0)\leq r^W.\end{equation}
...
Under the separability assumption above---restricted, per the discussion just given, to configurations in which at most one laboratory is active in any given segment, with rivals passive or inactive there---let $r^W$ be the wholesale value of a FLOP that clears the capacity market, $\sum_\ell\sum_k x_{\ell k}=K$. Then:
\begin{enumerate}
\item every active laboratory--segment pair satisfies $v_{\ell k}(x_{\ell k})=r^W$, and every inactive pair satisfies $v_{\ell k}(0)\leq r^W$;
\item segment $k$ is served by laboratory $L_k$ alone if and only if $v_{L_k,k}(0)>r^W$ \emph{and} $\max_{\ell\neq L_k}v_{\ell k}(0)\leq r^W$: the winner-take-all condition of this section applies segment by segment, with the market-wide clearing value $r^W$ in the role of the leader's marginal value of the final FLOP. The participation condition $v_{L_k,k}(0)>r^W$ is not redundant: without it, a segment that no laboratory serves at all---every pair inactive, including $L_k$---also satisfies $\max_{\ell\neq L_k}v_{\ell k}(0)\leq r^W$, so the condition on rivals alone is necessary but not sufficient for sole service by $L_k$; and
\item the identity of $L_k$ may differ across $k$. Following the aggregate condition of Section~\ref{sec:concentration} rather than re-invoking $r^W$---whose supporting value is not unique at a corner where one laboratory absorbs everything---one laboratory $L$ absorbs all of $K$ if and only if $v_L(K)\geq\max_k\max_{\ell\neq L}v_{\ell k}(0)$, where $v_L(K)\equiv G_L'(K)$ is the common marginal value laboratory $L$ attains when it optimally allocates $K$ across every segment it could serve, i.e., the multiplier on $\sum_k x_{Lk}=K$ in $\max_{\{x_{Lk}\}}\sum_k G_{Lk}(x_{Lk})$.
\end{enumerate}
\end{proposition}

The proof is the first-order condition of Section~\ref{sec:allocation} applied to the doubly indexed choice variables $x_{\ell k}$, with strict concavity of each $G_{\ell k}$ delivering uniqueness; parts~(2) and~(3) read off which pairs clear the participation threshold $r^A$, with part~(2) requiring in addition that $L_k$ itself clear it. Part~(2) is the boxed condition of this section restated within a segment: the segment leader's least valuable funded FLOP in segment $k$ earns exactly $r^W$, so sole service of the segment requires both that $L_k$ actually wants to serve it and that every rival's best use \emph{in that segment} be worth no more than that. Part~(3) mirrors the aggregate boxed condition of Section~\ref{sec:concentration} exactly, comparing $L$'s own marginal value of the last unit of $K$ directly against every rival's value at zero in every segment, rather than selecting a particular supporting $r^W$ at a degenerate corner where many prices support the same allocation.

The qualitative headline is part~(3): nothing forces $L_k$ to be constant across $k$. Laboratory $L$ may lead on coding and enterprise agents while laboratory $j$ leads on low-latency consumer queries or on a particular language, and both positions are consistent with the same clearing value $r^A$. Global winner-take-all now requires the demanding condition of this section to hold for the same laboratory in every segment at once, and it fails whenever any rival holds a protected niche---a segment where task-specific quality or rivals' frictions give it $v_{\ell k}(0)>r^W$. Four implications follow, of which the first two are direct consequences of Proposition~\ref{prop:segment-wta} and the last two are plausible readings of it rather than results.
...
The derivative is computed holding rivals' strategies fixed, the standard convention in the Nash-in-Nash bargaining framework of Appendix~\ref{app:bargaining}. Allowing rivals to re-equilibrate would move the value in a conduct-dependent direction: under quantity competition rivals contract when $\ell$ expands, raising the value of the marginal FLOP, while under differentiated price competition rivals cut prices, lowering it.

Three value concepts must now be kept distinct. Plain $v$ (with subscript $m$ or $\ell$) denotes the price-taking marginal net value per FLOP, $v_\ell=(P_\ell-b_\ell)/a_\ell$: the value a laboratory places on marginal compute when it treats the output price as unaffected by its own sales. This is the object that pins down the clearing value $r^W$ in Section~\ref{sec:concentration}, and the reading under which the formulas of Section~\ref{sec:allocation} were applied there, since with the price locally fixed $MR_m\simeq P_m$. The hatted $\widehat v_\ell$ is the laboratory's private marginal value under oligopoly, used in this section and afterward. The integrated benchmark prices the marginal FLOP at industry marginal revenue instead, and its value, $v^{\mathrm{int}}_\ell\equiv(MR_\ell-b_\ell)/a_\ell=c+\lambda$, is a third object, equal to neither. Evaluated at a common output vector, the three are ordered:

\begin{equation}c+\lambda=v^{\mathrm{int}}_\ell\;\leq\;\widehat v_\ell\;\leq\;v_\ell=\frac{P_\ell-b_\ell}{a_\ell}.\end{equation}

The right inequality holds because additional oligopoly output depresses the price earned on the laboratory's own inframarginal units, which a price taker ignores; it is strict whenever the laboratory produces. The left inequality is business stealing. A natural guess is that the oligopolist, being less than fully coordinated, values compute below the integrated coordinator as well; the guess is backwards whenever rivals produce, because the oligopolist ignores the price depression it inflicts on rivals' units while the coordinator internalizes it. Appendix~\ref{app:cournot} gives the linear closed forms, where the three values are $c+\lambda$, $r^W-BK/(a^2 n)$, and $r^W$, so the oligopoly value interpolates between the integrated and price-taking values as $n$ grows. Every use of $\widehat v_\ell\leq v_\ell$ below, for instance that laboratories bid less aggressively for marginal compute than price-taking providers would, is a comparison against the price-taking value.

Under a competitive upstream auction for fixed capacity, the wholesale compute price $r^A$\footnote{This is a distinct equilibrium object from the competitive-benchmark clearing value $r^W$ of Section~\ref{sec:concentration}. There, $r^W$ is pinned down by laboratories' \emph{competitive} (price-taking) marginal net value per FLOP; here, $r^A$ is the market-clearing compute price under the oligopoly/auction setting, pinned down by laboratories' \emph{private} marginal values $\widehat v_\ell$. The two coincide only in the limit where downstream market power vanishes; Appendix~\ref{app:cournot} gives the exact gap in the linear symmetric case, $r^W-r^A=BK/(a^2 n)$.} is approximately determined by the best losing laboratory's private marginal value:

\begin{equation}r^A\simeq\max\{c,\widehat v_F\}.\end{equation}
...
One special case sharpens the concentration condition of Section~\ref{sec:concentration}. With homogeneous demand and symmetric productivity, complete dominance is impossible under oligopoly conduct: a follower holding no compute has no inframarginal units to protect, so $\widehat v_F(0)$ equals the price-taking value $r^W$, while the leader's value at full capacity satisfies $\widehat v_L(K)<r^W$. Winner-take-all therefore requires productivity asymmetry or differentiation; symmetric laboratories cannot bid one another out of the market, however scarce compute becomes.
...
Price competition among differentiated laboratories should not be read as restricting output relative to the coordinated benchmark. The two-quality example of Section~\ref{sec:welfare} shows that competition can serve more of the market than the coordinator would---with capacity binding under duopoly where the coordinator would leave part of it idle---so the direction of the output distortion is not general; the robust object is the wedge between marginal revenue and marginal willingness to pay, not a claim that oligopoly contracts output.
...
\subsection{Welfare and the allocative wedge}
\label{sec:welfare}

Writing the welfare object explicitly sharpens what the joint-profit benchmark of Section~\ref{sec:allocation} does and does not deliver. Let $V(\mathbf y;\mathbf Q)$ denote gross purchaser value from the output vector $\mathbf y$---the area under the inverse-demand system, so that $\partial V/\partial y_m=P_m$, the marginal purchaser's willingness to pay, rather than marginal revenue $MR_m=\partial R/\partial y_m$. Social surplus net of resource cost is

\begin{equation}W(\mathbf y)=V(\mathbf y;\mathbf Q)-\sum_{m=1}^{M}b_m\cdot y_m-c\cdot\sum_{m=1}^{M}a_m\cdot y_m,\end{equation}

and the efficient allocation maximizes $W$ subject to the same capacity constraint $\sum_m a_m\cdot y_m\leq K$. With multiplier $\lambda^{\mathrm{eff}}$, every active model satisfies

\begin{equation}s_m\equiv\frac{P_m-b_m}{a_m}=c+\lambda^{\mathrm{eff}},\end{equation}

so efficiency equalizes marginal \emph{social} value per FLOP, $s_m$, which uses the price in place of marginal revenue. Against the joint-profit condition $v_m=(MR_m-b_m)/a_m=c+\lambda$, and because $MR_m\leq P_m$ with equality only under price-taking or perfect price discrimination,

\begin{equation}s_m-v_m=\frac{P_m-MR_m}{a_m}=\frac{\mu_m}{a_m}\geq0,\end{equation}

with $\mu_m$ the same downstream markup as in Section~\ref{sec:oligopoly}: the social value of a marginal FLOP weakly exceeds its private joint-profit value, and the gap is larger for models facing steeper, less elastic residual demand.
...
Under binding capacity the markup's welfare consequence changes form rather than disappearing. Total output is pinned by $K$, so the markup cannot reduce aggregate quantity; what it distorts instead is the allocation of compute across laboratories. Efficient use of scarce compute equalizes marginal social value per FLOP, but oligopolistic laboratories bid for compute according to their private marginal value $\widehat v_\ell$, which nets out the cannibalization of their own inframarginal output. When markups differ across laboratories---because they face different residual elasticities---the wedge between $\widehat v_\ell$ and social marginal value differs across laboratories too, so compute is drawn toward laboratories with smaller markups relative to the surplus-maximizing allocation. The deadweight loss of downstream market power therefore does not vanish when capacity binds; it is converted from an output-reduction loss into a cross-laboratory misallocation loss, in which the wrong models are served rather than too few outputs served in total. This is the allocative counterpart, on the compute-allocation margin, of the neutrality result of Proposition~\ref{prop:capacity-neutrality}: adding laboratories does not change total output, but heterogeneous markups still move compute away from its highest-value uses. Formally, when capacity binds and is fully used under both objectives, the profit benchmark and the efficient allocation exhaust the same $K$ but distribute it differently, and the resulting loss is purely allocative,

\begin{equation}\mathrm{DWL}=W(\mathbf y^{\mathrm{eff}})-W(\mathbf y^{*})\geq0,\end{equation}

vanishing only when $s_m$ is equalized across active models, which coincides with the profit benchmark's $v_m$-equalization only when all active models carry the same per-FLOP markup. Unlike the scarcity rent, this wedge does not disappear when capacity becomes slack: the markup keeps $s_\ell>v_\ell$ even at $r^A=c$, so abundant compute removes hyperscaler scarcity rents without restoring efficiency.

A two-quality example makes the wedge concrete, and shows that its direction under decentralized conduct is not what one might expect. Suppose two models share the same $a$ and $b$ and differ only in quality, $Q_L>Q_F$, with willingness to pay uniformly distributed. An integrated coordinator then serves only the high-quality model: versioning has no value when the lower-quality output burns the same FLOPs per unit for less purchaser value. Differentiated price competition between the two laboratories (Section~\ref{sec:oligopoly}) instead serves both, diverting part of a fixed $K$ to the low-quality model, and each FLOP so diverted produces less value than it would at the frontier model---a pure quality misallocation of the constrained input. The same example shows that oligopoly need not restrict output relative to the coordinated benchmark: duopoly competition can expand served demand beyond the coordinator's restriction, and capacity can bind under duopoly where the coordinator would have idled part of it. Oligopoly conduct can therefore either contract or expand output relative to the benchmark, depending on differentiation; the robust statement is the wedge between marginal revenue and marginal willingness to pay, not the direction of the output distortion.

\section{Vertical Integration and Foreclosure}
\label{sec:vertical-integration}
\label{sec:integration}

Everything to this point treats the two layers at arm's length: hyperscalers own $K$ and sell FLOPs, laboratories buy them at $r^W$ or $r^A$ or at a bargained price, and the interesting economics happens in the prices. The actual market is not organized this way, and the departures are not marginal. Microsoft has been simultaneously OpenAI's largest outside investor, for years its exclusive compute supplier and still the holder of a right of first refusal on new OpenAI capacity, and a claimant on a share of its revenue. Amazon and Google each hold multi-billion-dollar equity positions in Anthropic while supplying it with reserved capacity on their own accelerators, Trainium and TPUs respectively. Google DeepMind and Meta are the limiting case: the compute owner and the laboratory are the same firm, and no wholesale price for their internal flows exists even nominally. Custom silicon (TPUs, Trainium, Microsoft's Maia, OpenAI's announced co-designed parts) pushes further in the same direction by making part of the capital stock partially model-specific. The paper's central objects, the clearing prices $r^W$ and $r^A$, the bargaining weight $\zeta$ of Appendix~\ref{app:bargaining}, and the winner-take-all condition of Section~\ref{sec:concentration}, were all defined by transactions between independent parties. For the largest participants those transactions either do not occur or occur on terms set inside a relationship that the arm's-length model does not describe. This section models the difference; the allocation rule survives it, but the division of rents largely ceases to be a market outcome.

\subsection{The integrated entity and the internal shadow price}
\label{sec:integration-internal}

Suppose hyperscaler $h$ and laboratory $\ell$ are under common ownership. For exposition, let laboratory $\ell$ draw compute only from $h$; the general case adds outside purchases without changing the logic. The integrated entity chooses its own downstream output $y_\ell$ and merchant sales $x_{hj}$ to rival laboratories $j$ at wholesale prices $r_j$, subject to its capacity:

\begin{equation}\Pi_{h\ell}=\left[P_\ell(y_\ell,\mathbf y_{-\ell})-b_\ell\right]\cdot y_\ell+\sum_{j\neq\ell}r_j\cdot x_{hj}-c\cdot\Big(a_\ell\cdot y_\ell+\sum_{j\neq\ell}x_{hj}\Big)\quad\text{subject to}\quad a_\ell\cdot y_\ell+\sum_{j\neq\ell}x_{hj}\leq K_h.\end{equation}

The first observation is bookkeeping, but it matters. Any internal transfer price $\tilde r$ applied to the flow $x_{h\ell}=a_\ell\cdot y_\ell$ enters this objective twice with opposite signs and cancels. Within the integrated entity, the wholesale price of compute is an accounting object, chosen for tax, cost-allocation, or investor-reporting purposes, not a market price. What disciplines the internal allocation is the shadow value of the capacity constraint. Let $\lambda_h\geq0$ be its multiplier. Internal use expands until the laboratory's private marginal value per FLOP, in the sense of Section~\ref{sec:oligopoly}, equals the full opportunity cost:

\begin{equation}\widehat v_\ell=c+\lambda_h.\end{equation}

The allocation rule of Section~\ref{sec:allocation} thus survives integration intact---the marginal FLOP still flows to its highest private value against the same shadow price---which is why the integration gap bears far more heavily on the division of rents than on the allocation. Merchant sales are different, because supplying a rival is not neutral for the entity's own downstream business. Define the \emph{diversion harm} to the affiliated laboratory per FLOP supplied to rival $j$ as

\begin{equation}\chi_{\ell j}\equiv-\frac{y_\ell}{a_j}\cdot\frac{\partial P_\ell}{\partial y_j}\geq0.\end{equation}

This is the cross-model analogue of the markup $\mu_\ell=-y_\ell\cdot\partial P_\ell/\partial y_\ell$: it measures the erosion of the affiliated laboratory's inframarginal revenue when rival $j$ converts one more FLOP into output. The merchant first-order condition is then

\begin{equation}r_j-\chi_{\ell j}=c+\lambda_h,\end{equation}

so the integrated entity supplies rivals only at a premium $\chi_{\ell j}$ over its internal shadow value. Even an integrated firm with an active merchant business, Google Cloud selling TPU capacity or Microsoft selling Azure GPU time, therefore posts external prices that overstate its internal opportunity cost of compute whenever its own laboratory competes with the buyer.

For the affiliated laboratory, the three-part price decomposition of Section~\ref{sec:oligopoly} collapses. Substituting the internal allocation condition into the pricing problem,

\begin{equation}P_\ell=\underbrace{b_\ell+a_\ell\cdot c}_{\text{physical production cost}}+\underbrace{a_\ell\cdot\lambda_h+\mu_\ell}_{\text{integrated margin}}.\end{equation}

The middle term of the original decomposition, $a_\ell\cdot(r_\ell-c)$, was a transaction: money that actually moved from a laboratory to a hyperscaler. Here it is replaced by $a_\ell\cdot\lambda_h$, a shadow value that no transaction reveals, folded together with the downstream markup into a single integrated margin. This has an identification consequence. For integrated or affiliated pairs, the split between hyperscaler scarcity rent and model-provider markup is not recoverable from observed prices; only the sum $P_\ell-b_\ell-a_\ell\cdot c$ is. Reported internal compute pricing, such as the discounted Azure rates OpenAI has been reported to pay, measures a transfer-pricing choice rather than $r^A$, and empirical work that treats such figures as market prices of compute will mismeasure both rents. The decomposition survives as an economic statement about opportunity costs, but not as an accounting of who is paid what.

\subsection{Foreclosure and raising rivals' costs}
\label{sec:integration-foreclosure}

The merchant condition above already contains the foreclosure result; it remains only to state it against the right benchmark. A non-integrated hyperscaler allocates a marginal FLOP to whichever laboratory values it most: it supplies rival $j$ rather than laboratory $\ell$ whenever $\widehat v_j\geq\widehat v_\ell$. The integrated entity compares the same options differently. Internal use is worth $\widehat v_\ell-c$. Selling to rival $j$ at price $r_j$ is worth $r_j-c-\chi_{\ell j}$, because the entity internalizes the harm to its own downstream position. Even if the rival can be charged its full willingness to pay, $r_j=\widehat v_j$, the integrated entity withholds the FLOP whenever

\begin{equation}\boxed{\widehat v_j<\widehat v_\ell+\chi_{\ell j}.}\end{equation}

This is the classic raising-rivals'-cost trade-off written in the model's objects: the forgone merchant margin $\widehat v_j-c$ is weighed against the internal value $\widehat v_\ell-c$ \emph{plus} the competitive relief $\chi_{\ell j}$ from starving a rival. The integrated entity refuses sales that a merchant hyperscaler would happily make, at prices the rival would willingly pay, whenever the rival's marginal value exceeds the affiliated laboratory's by less than the diversion harm. If the rival can be charged only a bargained share of its value rather than all of it, as in Appendix~\ref{app:bargaining}, the foreclosure region is wider still. Short of outright refusal, the same wedge operates through prices: the integrated entity's reservation wholesale price to rivals is $c+\lambda_h+\chi_{\ell j}$, above the $\bar v_h$ a merchant would demand, so rivals that are served pay more.

Two features of $\chi_{\ell j}$ do most of the work in what follows. It is large precisely when the rival is a close substitute at scale, so that rival output presses directly on the affiliated laboratory's residual demand, and small when the rival is differentiated by task, segment, language, or integration. The same horizontal differentiation that produces coexistence under arm's-length allocation in Section~\ref{sec:concentration} therefore also shrinks the incentive to foreclose under integration: the laboratories most worth foreclosing are the ones hardest to distinguish from one's own. And $\chi_{\ell j}$ is essentially the object that upward-pricing-pressure diagnostics attempt to measure in merger review, a diversion ratio multiplied by a margin, which is convenient because it means the foreclosure condition is stated in terms competition authorities already try to estimate.

The condition governs a marginal block of $K_h$, not the market. A rival denied compute by one integrated hyperscaler can buy from merchant suppliers (Oracle, CoreWeave, the growing tier of specialist GPU clouds) or from a hyperscaler allied with a different laboratory. Foreclosure binds market-wide only to the extent that integrated and allied capacity is a large share of frontier-grade $K$, which, at present, it is.

\subsection{Consequences: allocation, rents, dominance, and consumer prices}
\label{sec:integration-consequences}

\paragraph{Allocation of $K$.} Under arm's-length allocation, active laboratories' private marginal values are equalized at $r^A$. Under integration they are not: a wedge of up to $\chi_{\ell j}$ separates the marginal value of the affiliated laboratory from that of rivals served (or refused) by the same hyperscaler. Compute can remain with an affiliated laboratory even when a rival values the marginal FLOP more. Whether this misallocation is also a \emph{social} loss inherits the caveat of Section~\ref{sec:welfare}: private values $\widehat v$ already diverge from social values by the markup term, and the model does not restrict which distortion dominates.

\paragraph{The rent split.} For internal flows the bargaining problem of Appendix~\ref{app:bargaining} simply disappears: $\zeta$ is irrelevant where there is no counterparty, and the division of rent between the hyperscaler and laboratory arms is an accounting choice. For external flows, the integrated supplier's reservation price rises by $\chi_{\ell j}$, transferring rent from rival laboratories to the integrated entity. Observed wholesale prices therefore bifurcate: rivals pay more than the merchant benchmark, affiliates pay whatever the transfer-pricing convention says, and neither observation identifies $r^A$.

\paragraph{Dominance and coexistence.} The winner-take-all condition of Section~\ref{sec:concentration} shifts, and the direction depends entirely on who owns the capacity. If the leading laboratory $L$ is integrated with the hyperscalers controlling the constrained capacity, rivals must clear not $r^A$ but the diversion-inclusive reservation price, and complete dominance requires only

\begin{equation}\widehat v_L(K)\geq\max_{j\neq L}\left[\widehat v_j(0)-\chi_{Lj}\right],\end{equation}

a weakening of the arm's-length condition by exactly the diversion harm. Dominance can then occur even where the demanding condition of Section~\ref{sec:concentration} fails. But the logic is symmetric. If capacity is instead partitioned among several hyperscalers, each integrated with or invested in its own laboratory, then the leader can pry a rival's patron capacity loose only by compensating the patron for its own laboratory's loss: dominance requires

\begin{equation}\widehat v_L(K)\geq\max_{j\neq L}\left[\widehat v_j(0)+\chi_{jL}\right],\end{equation}

which is strictly \emph{harder} than the arm's-length condition. Integration thus tilts the market toward dominance when one alliance controls the constrained capacity, and toward entrenched coexistence when several do. The present configuration, Microsoft aligned with OpenAI, Amazon and Google with Anthropic, Google and Meta operating in-house, is the second case, and this may be part of why several frontier laboratories coexist despite what look like meaningful capability gaps: each laboratory's access to its patron's capacity is insulated from the leader's bidding. The model does not predict which configuration prevails; it says the configuration matters, in a signed and quantifiable way.

\paragraph{Consumer prices.} Integration pushes in both directions. The benign channel is the elimination of double marginalization: under arm's-length linear pricing, the affiliated laboratory's perceived marginal cost includes a bargained wholesale margin, $r_\ell>c+\lambda_h$, on top of which it adds its own markup; integration replaces $r_\ell$ with the true opportunity cost $c+\lambda_h$, weakly expanding the affiliated laboratory's output and lowering its price. The adverse channel is that foreclosure raises rivals' input price $r_j$, which raises their prices and, through strategic complementarity, relaxes the pricing constraint on the affiliated laboratory as well. When capacity binds and is fully utilized, the homogeneous-good example of Appendix~\ref{app:cournot} is a useful discipline: there the consumer price depends on $K$ alone, so foreclosure that merely reallocates fully-used capacity redistributes rents without moving the average price. The harm then shows up in quality-adjusted rather than nominal terms, as compute held at a lower-$q$ affiliated model, or, in the extreme, as capacity idled because withholding is worth more than selling. When capacity is slack, the Cournot discipline disappears and raising rivals' costs translates directly into higher rival prices through the Lerner condition of Section~\ref{sec:oligopoly}. The net effect is conditional: integration lowers the affiliated product's price and raises rivals' prices, and which dominates depends on capacity utilization and substitution patterns.

\subsection{Partial integration: equity stakes and reserved capacity}
\label{sec:integration-partial}

Full common ownership is the limiting case, but not the modal deal. The observed structures are equity stakes combined with preferential or reserved capacity and some degree of exclusivity: Microsoft's investment in and revenue share from OpenAI, Amazon's and Google's stakes in Anthropic alongside committed Trainium and TPU capacity. These interpolate between arm's length and integration in two ways the model can capture separately.

\paragraph{Equity as an effective bargaining weight.} Let the hyperscaler hold an equity share $\omega\in[0,1]$ in the laboratory. In the Nash bargain of Appendix~\ref{app:bargaining}, the hyperscaler's payoff from a negotiated price $r$ becomes $(r-\bar v_h)+\omega\cdot(\widehat v_\ell-r)$: it now values the laboratory's surplus at rate $\omega$. The bargaining program is

\begin{equation}\max_{r}\;\left(\widehat v_\ell-r\right)^{\zeta}\cdot\left[\left(r-\bar v_h\right)+\omega\cdot\left(\widehat v_\ell-r\right)\right]^{1-\zeta},\end{equation}

and its solution leaves the laboratory a face-value surplus

\begin{equation}\widehat v_\ell-r_{\ell h}(\omega)=\frac{\zeta}{1-\omega}\cdot\left(\widehat v_\ell-\bar v_h\right).\end{equation}

An equity stake $\omega$ is therefore exactly equivalent, for the negotiated price, to raising the laboratory's bargaining weight from $\zeta$ to $\zeta/(1-\omega)$. Once $\zeta>1-\omega$, the wholesale price falls \emph{below} the hyperscaler's outside option $\bar v_h$: the hyperscaler sells at less than its opportunity cost because it recoups through the equity, which is the model's rationalization of the preferential compute pricing these partnerships are reported to include. Meanwhile the hyperscaler's all-in compensation, wholesale margin plus equity share of the laboratory's surplus, is

\begin{equation}\left(r_{\ell h}-\bar v_h\right)+\omega\cdot\left(\widehat v_\ell-r_{\ell h}\right)=\left(1-\zeta\right)\cdot\left(\widehat v_\ell-\bar v_h\right),\end{equation}

independent of $\omega$. Holding $\zeta$ fixed, equity changes what wholesale prices \emph{measure}, not the true division of surplus, which is a further reason to distrust observed compute prices within partnerships. (In practice the stake plausibly also shifts $\zeta$ itself, through exclusivity and relationship-specific investment; the model treats that as a parameter change rather than deriving it.) On the merchant side, the stake scales the foreclosure incentive linearly: the partially integrated hyperscaler's reservation price to rival $j$ becomes

\begin{equation}r_j\geq\bar v_h+\omega\cdot\chi_{\ell j},\end{equation}

recovering the merchant benchmark at $\omega=0$ and the full-integration condition at $\omega=1$. As $\omega\rightarrow1$ the negotiated internal price loses meaning entirely, which is the transfer-price result of Section~\ref{sec:integration-internal} approached continuously.

\paragraph{Reserved capacity.} The second instrument is a capacity carve-out: a block $K_R$ committed to the affiliated laboratory, at contractually fixed terms, before the residual market clears. The residual market then allocates $K-K_R$ among the remaining claimants, and because rivals' aggregate marginal-value schedule slopes downward,

\begin{equation}r^A(K-K_R)\geq r^A(K),\end{equation}

with strict inequality when capacity binds. This is raising rivals' costs without any refusal to deal: no rival is denied service, yet every rival pays more, and the effect operates through market clearing rather than through any individually attributable act. The affiliated laboratory's contractual price and its opportunity cost now differ, which is the distinction between $r_j$ and $r^W$ that Section~\ref{sec:trailing-labs} develops, here created deliberately by contract rather than inherited from history. Rights of first refusal and exclusivity clauses, such as Microsoft's right of first refusal over new OpenAI capacity, function as state-contingent reservations: they need never be exercised to matter, since their effect is on the capacity rivals can expect to bid for. A reservation plus a large equity stake reproduces most of full integration's allocation effects while leaving formal control, and with it merger jurisdiction, untouched, which is presumably part of why the deals are structured this way.

\subsection{Heterogeneous hyperscalers}
\label{sec:integration-hetero}

Arm's length and full integration are endpoints, and the supply side between them is itself heterogeneous. A natural reduced-form extension replaces the representative hyperscaler with heterogeneous suppliers $h$, each characterized by capacity $K_h$, operating cost $c_h$, and an outside- or internal-demand parameter $o_h$. This captures neoclouds, large diversified clouds, captive internal demand, and cost differences across hardware vintages without modeling the semiconductor supply chain: the single clearing price $r^A$ is then replaced by a supply schedule traced out by the ordered marginal costs and outside options of the individual suppliers, and vertical integration appears as a supplier whose $o_h$ reflects captive demand from an affiliated laboratory rather than an external market price.

\subsection{Implications for competition policy}
\label{sec:integration-policy}

This is the live foreclosure question for competition authorities: the FTC's 6(b) study of the Microsoft--OpenAI, Amazon--Anthropic, and Google--Anthropic partnerships and the CMA's review of Microsoft--OpenAI both turned on whether equity-plus-compute arrangements amount to control and whether they can be used to disadvantage rivals. The model does not settle those cases, but it does say which quantities settle them. Integration is benign where its effects run through the elimination of double marginalization, through securing demand for irreversible capacity investment (the investment condition of Section~\ref{sec:model-race} makes expected utilization the driver of capital expenditure, and a committed downstream partner raises it), and through the co-design of silicon and models that lowers $a_\ell$ or $c$ outright. It is anticompetitive where the diversion term $\chi_{\ell j}$ is large and the rival's outside options are thin: the foreclosure condition binds when the affiliated and rival laboratories are close substitutes at scale, and it binds market-wide only when integrated and allied capacity is a large share of the frontier-grade total, so that merchant suppliers cannot arbitrage the withheld FLOPs. Those are, respectively, a demand-side and a supply-side question that existing merger tools, diversion ratios and margins on one side, capacity shares and entry conditions on the other, are already built to ask. The model adds one structural observation the enforcement debate tends to miss: with several integrated stacks rather than one, the same wedge that would entrench a dominant alliance instead entrenches coexistence, because each laboratory's patron internalizes the cost of selling it out. Whether that equilibrium is durable depends on capacity shares and on whether the alliances persist, neither of which the model pins down; but it cautions against reading vertical integration in this market as pointing uniformly toward monopolization.

\section{Trailing Laboratories}
\label{sec:trailing-labs}

Let $r_j$ denote trailing laboratory $j$'s actual contractual payment per FLOP, and let $r^W$ denote the current market value or opportunity cost of a FLOP. These may differ when compute was prepaid, obtained under a long-term contract, or cannot be transferred or resold.
...
\begin{equation}\frac{MR_j-b_j}{a_j}\geq r^W.\end{equation}
...
\begin{equation}v_j(0)<r^W\end{equation}
...
\begin{equation}r_j+\frac{b_j}{a_j}<p_j<r^W+\frac{b_j}{a_j}.\end{equation}
...
Let $\Lambda_L$ be the hazard that a competitor releases a sufficiently strong substitute. With discount rate $\rho$ (distinguished here from the wholesale compute prices $r^W$, $r^A$ of Sections~\ref{sec:concentration}--\ref{sec:oligopoly}, which play no role in this formula), the approximate value of temporary leadership is
...
\end{enumerate}
Consequently a breakthrough that widens the gap lowers $\epsilon_{LL}$ and raises $\mu_L$ (part~1) and $\widehat v_L-\widehat v_F$ (part~2); a follower release that narrows the gap reverses both. Part~2 holds the compute-inclusive cost $\kappa$ fixed as $\Delta$ varies; it is a ceteris-paribus, partial-equilibrium statement about the immediate effect of a quality change, not a claim that $\kappa$ (through $r^A$) stays fixed once the market re-equilibrates around a follower's release---see the discussion following the proof in Appendix~\ref{app:two-model} for why the partial-equilibrium version remains informative despite this.
\end{proposition}
...
Discount flow profits at rate $\rho$; if a laboratory's rents also face a displacement hazard $\Lambda$, discount at the effective rate $\rho+\Lambda$, by the same hazard-perpetuity logic as the temporary-leadership value of Section~\ref{sec:model-race}. Free entry then pins down the number of laboratories: this is the endogenous-sunk-cost logic of \citet{sutton1991}, applied with training compute as the sunk outlay. The flow profit $\pi(n)$ used below is the competitive-upstream Cournot profit of Appendix~\ref{app:cournot}, so this proposition inherits the same price-taking-in-$r^A$ conduct assumption as Proposition~\ref{prop:capacity-neutrality}, with the same qualification: under Nash bargaining with laboratory weight $\zeta>0$ (Appendix~\ref{app:bargaining}), laboratories retain part of the scarcity rent that price-taking here transfers upstream, $\pi(n)$ is correspondingly larger, and the free-entry count $n^*$ below is a lower bound on the bargained-conduct count rather than an estimate of it.
...
These relationships hold across equilibrium concepts that differ sharply in what they predict, and the difference is worth dwelling on. Appendix~\ref{app:bargaining} shows that, conditional on conduct, every wholesale-price object in the paper is an instance of one negotiated-price equation; what the unification leaves free is conduct itself. The same technology, the same capacity, and the same demand are consistent with competitive commoditization, with a stable differentiated oligopoly, and with concentrated foreclosure, depending on whether laboratories take compute prices as given or restrict demand, whether hyperscalers serve all comers or reserve capacity for affiliates, and whether integration ties one alliance or several to the constrained capital stock (Section~\ref{sec:integration}). The model does not predict which conduct prevails, and it would overreach to claim that regulation determines the outcome. What it does say is that conduct is the selecting variable, and conduct is what competition policy acts on: the levers that move the market among these equilibria---exclusivity, reserved capacity, equity stakes, refusals to deal, coordinated purchasing---are the ones the FTC and CMA reviews discussed in Section~\ref{sec:integration-policy} are already examining. Market structure in this industry is therefore less technologically determined than discussions of AI concentration tend to assume; within wide bounds, it is a question of how firms are permitted to behave.
...
Two conventions behind this formulation should be explicit. First, $\bar v_h=\max\{c,\widehat v_F\}$ treats the disagreement point as the redeployment value of a marginal block, holding the rest of the downstream equilibrium fixed. For infinitesimal blocks this is exact, by the envelope theorem. For the lumpy, multi-year contracts that actually characterize hyperscaler deals it is an approximation: if a laboratory loses a discrete block, rivals expand, prices re-equilibrate, and both parties' disagreement payoffs move, so a full treatment in the manner of \citet{hornWolinsky1988} would recompute the downstream equilibrium at each disagreement point. Second, in any interior clearing equilibrium all active laboratories' private values are equalized at $r^A$, so the match surplus on the marginal block is zero and $\zeta$ is irrelevant at the margin: the negotiated price is $r^A$ for every bargaining weight. The weight bites on inframarginal or lumpy blocks and at corner allocations. Put differently, conduct pins the marginal price of compute; $\zeta$ divides the inframarginal rents.

If only a few laboratories purchase frontier compute, they may exercise oligopsony. They can reduce the wholesale compute price below the competitive-auction benchmark by restricting or coordinating demand, subject to the hyperscaler's outside options. This is a change in conduct, not in bargaining weight: quantity withholding changes how private values map into the clearing price, which no value of $\zeta$ in the bilateral problem reproduces. A market with few laboratories and few hyperscalers is therefore a bilateral oligopoly: downstream oligopoly raises purchaser prices, while upstream oligopsony transfers part of the compute scarcity rent from hyperscalers to laboratories.

\subsection*{One price equation, many market structures}

The negotiated-price formula organizes the paper's other wholesale-price objects as well. Conditional on downstream conduct, each is the Nash-in-Nash price $r=\zeta\cdot\bar v_h+(1-\zeta)\cdot\widehat v$ for an appropriate pair $(\zeta,\bar v_h)$, with the private value $\widehat v$ indexed by conduct: price-taking conduct yields $v_\ell$ and hence $r^W$, Cournot or differentiated-price conduct yields the $\widehat v_\ell$ that clears at $r^A$, and joint-profit conduct---a single negotiating unit internalizing all cross-effects---yields the integrated value $c+\lambda$, at which point $\zeta$ only divides rent, consistent with the observation in Section~\ref{sec:allocation} that transfers do not affect the joint-profit objective. The competitive-auction benchmark is $\zeta\rightarrow1$ with the outside option at the best losing bidder's value; $\zeta\rightarrow0$ is full extraction by a perfectly discriminating upstream monopolist. The Walrasian price requires two limits at once: conduct approaching price-taking, so that $\widehat v\rightarrow v_\ell$, and enough upstream competition that $\bar v_h$ approaches the clearing value. Neither alone suffices. In the linear example of Appendix~\ref{app:cournot} the conduct-indexed values are totally ordered, $c+\lambda=r^A_{(1)}\leq r^A_{(n)}\leq r^W$, converging at rate $1/n$.

This unifies the division of rents, not the allocation. Conduct remains a free parameter of the frame: nothing in the bargaining problem selects among price-taking, Cournot, differentiated price competition, coordination, or the oligopsony deviation above, and under asymmetry or differentiation these conducts allocate $K$ differently, not merely split the rents differently. Section~\ref{sec:implications} draws the interpretive consequence.
...
Provider $m$ offers a subscription contract $(\phi_m,P_m^{\mathrm{sub}})$, where $\phi_m\geq0$ is a fixed subscription fee and $P_m^{\mathrm{sub}}$ is the marginal price of a standardized output after subscribing. A non-subscriber may instead pay spot price $P_m^{\mathrm{spot}}$, with

\begin{equation}P_m^{\mathrm{sub}}<P_m^{\mathrm{spot}}.\end{equation}
...
\begin{equation}\left(P_m^{\mathrm{spot}}-P_m^{\mathrm{sub}}\right)\cdot\mathbb E[y_{im}]\geq\phi_m.\end{equation}
...
\begin{equation}V_{imk}=\theta_i\cdot Q_{mk}-P_m^{\mathrm{sub}}.\end{equation}
...
\begin{equation}B_{ik}(S)=\max\left\{0,\max_{m\in S}\left(\theta_i\cdot Q_{mk}-P_m^{\mathrm{sub}}\right)\right\}.\end{equation}

Under deterministic quality, the incremental value of adding provider $j$ is

\begin{equation}\Omega_{ij}(S)=\sum_k T_{ik}\cdot\left[\theta_i\cdot Q_{jk}-P_j^{\mathrm{sub}}-B_{ik}(S)\right]_+-E_{ij},\qquad [z]_+\equiv\max\{z,0\},\end{equation}
...
\begin{equation}\Omega_{ij}(S)=\sum_k T_{ik}\cdot\mathbb E\left[\theta_i\cdot Q_{ijk}-P_j^{\mathrm{sub}}-B_{ik}(S)\right]_+-E_{ij}.\end{equation}
...
\begin{equation}\Pi_m=\phi_m\cdot n_m+\left(P_m^{\mathrm{sub}}-\kappa_m\right)\cdot Y_m-C_m,\end{equation}

where $C_m$ is provider $m$'s fixed cost per period.

A provider may set $P_m^{\mathrm{sub}}<\kappa_m$ if fixed subscription revenue covers the usage subsidy:

\begin{equation}\phi_m\cdot n_m\geq\left(\kappa_m-P_m^{\mathrm{sub}}\right)\cdot Y_m+C_m.\end{equation}
...
\begin{equation}r^A=\frac{A-b}{a}-\frac{B\cdot(n+1)\cdot K}{a^2\cdot n}=r^W-\frac{B\cdot K}{a^2\cdot n},\end{equation}

where $r^W=(A-b)/a-B\cdot K/a^2$ is the price-taking clearing value of Section~\ref{sec:concentration}, the $n\to\infty$ limit of $r^A$. The gap $r^W-r^A=B\cdot K/(a^2\cdot n)$ closes at rate $1/n$, and the integrated benchmark's internal value is the $n=1$ case, $c+\lambda=r^A_{(1)}=r^W-B\cdot K/a^2$; the three values of Section~\ref{sec:oligopoly} are therefore ordered $c+\lambda=r^A_{(1)}\leq r^A_{(n)}\leq r^W$. Consumer price is
...
Above this threshold, capacity is slack, $r^A=c$, hyperscaler scarcity rents vanish, and laboratories retain ordinary Cournot markups. Appendix~\ref{app:slack-capacity}, next, takes up that slack case in full.

\section{The Slack-Capacity Regime}
\label{app:slack-capacity}

The main text, from Section~\ref{sec:fixed-capacity} onward, maintains the scarcity assumption: desired output at prices near physical production cost exceeds available capacity, the multiplier $\lambda$ is positive, and the wholesale price of compute carries a scarcity premium, $r^A>c$. The opposite regime---capacity slack, $\lambda=0$, $r^A=c$---appears in the main text only as asides attached to binding-case results: the slack branch of Section~\ref{sec:oligopoly}, the closing sentence of Proposition~\ref{prop:capacity-neutrality}'s discussion, part~(4) of Proposition~\ref{prop:free-entry}, and the scope restriction around the Bertrand analysis of Section~\ref{subsec:price-competition}. This appendix works the regime out in one place. It introduces no new primitives and no new notation; exact statements use the symmetric linear-demand system of Appendix~\ref{app:cournot}, which is the closest sibling of everything derived here. Following the standard of Section~\ref{subsec:entry-honesty}, each subsection states what is derived (within that linear system), what is a qualitative implication of derived objects, and what is left open.
...
Wholesale price $r^A$ & $\frac{A-b}{a}-\frac{B\cdot(n+1)\cdot K}{a^{2}\cdot n}>c$ & $c$ \\
...
\begin{equation}\lambda(K)=r^{A}(K)-c=\frac{B\cdot(n+1)}{a^{2}\cdot n}\cdot\left(\frac{n}{n+1}\cdot\bar K-K\right),\end{equation}
...
Proposition~\ref{prop:segmented-entry} priced niche protection under binding capacity: the recurring friction had to absorb the full segment markup, $s_{\min}=B\cdot K/a$, and part of what made that markup no larger was that a rival's alternative to invading was selling output elsewhere at the scarcity price $r^{A}>c$. Under slack capacity the same three-line derivation runs with $r=c$. A segment monopolist facing marginal cost $b+a\cdot c$ sets $A-2\cdot S\cdot B\cdot y_k=b+a\cdot c$, so
...
with the markup $(A-b-a\cdot c)/2$ now the unconstrained monopoly markup, independent of both $S$ and $K$. The protection condition is the same participation logic as in the binding case: a rival's first unit in segment $k$ is unprofitable when $(P_k^{\mathrm{slack}}-s-b)/a\leq r^{A}=c$, which rearranges to
...
This formula holds at any allocation $(x_L,x_F)$, not only at a Nash equilibrium of the capacity game; we evaluate it at the winner-take-all corner $x_L=K$, $x_F=0$, which is the case the paper's own definition of $r^A\simeq\max\{c,\widehat v_F\}$ (Section~\ref{sec:oligopoly}) already presumes, since that expression compares the leader's marginal value to the \emph{best losing} laboratory's value at zero. At this corner,
...
where the strategic term $y_F\cdot\partial P_F/\partial y_F$ drops out of $\widehat v_F$ because $y_F=0$: the follower's value at the margin is just its price net of cost, with no cannibalization term, matching the inactive-laboratory participation condition---a follower's value at zero cannot exceed the clearing price---used throughout Sections~\ref{sec:concentration}--\ref{sec:trailing-labs}. Subtracting, and writing $Q_F=Q_L-\Delta$,
